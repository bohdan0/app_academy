...
class MetaCorgiSnacks
  ...
  #   # Your code goes here...
  #   str_name = name.to_s
  #   if ['bone', 'kibble', 'treat'].include?(str_name)
  #     get_info = "get_#{str_name}_info".to_sym
  #     info = @snack_box.send(get_info, @box_id)
  #
  #     get_tastiness = "get_#{str_name}_tastiness".to_sym
  ...
  def self.define_snack(name)
    ...
    define_method(name) do
      ...
      result = "#{str_name.capitalize}: #{info}: #{tastiness} "
      tastiness > 30 ? "* #{result}" : result
    end
  end
end
...
# box = SnackBox.new
# meta = MetaCorgiSnacks.new(box, 1)
